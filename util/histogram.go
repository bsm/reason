...
// NewHistograms inits a new numeric streams distribution.
func NewHistograms(maxBins uint32) *Histograms {
	return &Histograms{Cap: maxBins}
}

// Observe adds a new observation.
func (s *Histograms) Observe(cat int, value float64) {
	s.ObserveWeight(cat, value, 1.0)
}

// ObserveWeight adds a new observation with a weight.
func (s *Histograms) ObserveWeight(cat int, value, weight float64) {
	if cat < 0 || math.IsNaN(value) || math.IsInf(value, 0) || weight <= 0 {
		return
	}

	if n := cat + 1; n > cap(s.Data) {
		data := make([]Histogram, n, 2*n)
		copy(data, s.Data)
		s.Data = data
	} else if m := len(s.Data); n > m {
		s.Data = s.Data[:n]
	}
	s.Data[cat].Cap = s.Cap
	s.Data[cat].ObserveWeight(value, weight)
}

// NumRows returns the number of rows, including blanks.
func (s *Histograms) NumRows() int {
	return len(s.Data)
}

// NumCategories returns the number of categories.
func (s *Histograms) NumCategories() int {
	n := 0
...
func (s *Histograms) WeightSum() float64 {
	sum := 0.0
	for _, h := range s.Data {
		sum += h.Weight
	}
...
// At returns the histogram at the given cat (or nil).
func (s *Histograms) At(cat int) *Histogram {
	if cat > -1 && cat < len(s.Data) {
		if h := s.Data[cat]; h.Weight > 0 {
			return &h
		}
	}
	return nil
